...
internals.utility = {
  ...
  formatResponse (event, tags) {
    const query = util.format(event.query)
  ...
  formatDefault (event, tags) {
    const data = typeof event.data === 'object' ? util.format(event.data) : event.data
...
module.exports = class GoodStreamAdapter extends Stream.Writable {
  ...
  _write (data, encoding, callback) {
    ...
    if (eventName) {
      method = this._streamAdapterOptions.goodEventLevels[eventName] || 'info'

      // Process hapi server event
      let tags = []
      ...
      if (eventName === 'error' || data.error instanceof Error) {
        ...
        if (data.method) {
          ...
          output.context = {
            action: `${data.method.toUpperCase()} ${path}`
          }
        }
        method = 'error'
      } else if (eventName === 'ops') {
        output = internals.utility.formatOps(data, tags)
      } else if (eventName === 'response') {
        output = internals.utility.formatResponse(data, tags)

        // if (data.statusCode && data.statusCode > 499) {
        //   output = new Error(output)
        //   output.stack = null
        //   output.params = data.query
        //   output.context = {
        //     action: `${data.method.toUpperCase()} ${data.path}`,
        //     userAddr: data.source.remoteAddress,
        //     userAgent: data.source.userAgent,
        //     hostname: data.instance
        //
        //   }
        //   method = 'error'
        // }
        ...
        output = internals.utility.formatDefault(data, tags)
      }
    }
